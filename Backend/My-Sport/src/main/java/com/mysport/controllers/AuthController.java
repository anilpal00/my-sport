...
import com.mysport.payload.request.SignupRequest;
import com.mysport.payload.response.JwtResponse;
import com.mysport.payload.response.MessageResponse;
...
import com.mysport.security.services.UserDetailsImpl;
...
public class AuthController {
	...
	public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
		...
		user.setRoles(roles);
		int aNumber = (int) ((Math.random() * 900) + 100); 	 
		String msId = "MS-" + aNumber;
		user.setMsID(msId);
		userRepository.save(user);
		return ResponseEntity.ok(new MessageResponse("Player registered successfully!"));
//		return new ResponseEntity<>(msId, HttpStatus.OK);
	}
...
//		return new ResponseEntity<>(updatedUserProfile, HttpStatus.CREATED);
	}
